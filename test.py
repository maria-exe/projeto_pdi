...
def run_pipeline():
    data_dir = r"c:\Users\crisn\Downloads\archive\redEyeRemoval\data"
    output_dir = r"c:\Users\crisn\Desktop\projpdi\projeto_pdi\output"
    ...
    for filename in images:
        ...
        cv2.imwrite(os.path.join(output_dir, f"{base_name}_Y.png"), normalize_channel(Y))
        cv2.imwrite(os.path.join(output_dir, f"{base_name}_Cg_prime.png"), normalize_channel(Cg_prime))
        cv2.imwrite(os.path.join(output_dir, f"{base_name}_Cr_prime.png"), normalize_channel(Cr_prime))
        cv2.imwrite(os.path.join(output_dir, f"{base_name}_skin_mask.png"), (skin_mask * 255).astype(np.uint8))
        ...
        for idx, info in enumerate(faces_info):
            ...
            cv2.imwrite(os.path.join(output_dir, f"{base_name}_face_{idx}_rf_crop.png"), rf_crop)
            ...
            cv2.imwrite(os.path.join(output_dir, f"{base_name}_face_{idx}_redness.png"), redness_vis)
            
        cv2.imwrite(os.path.join(output_dir, f"{base_name}_vis_detection.png"), img_vis)
